...
def changePage(root_class,numpage=1):
    link = 'http://www.chinatimes.com/history-by-date/'+root_class+'?page='+str(numpage)
    print(link)

    Page = rq.get(link)#目標網佔
...
def catch_text_from_page(Text_area,f,tag):
    ...
    try:
        ...
        f.write(f_Text_area)
        catch_text_from_page(Text_area[f_Text_area_End+3:],f,tag)
    except ValueError as e:
        print('Text end'+str(tag))
    except Exception as e:
        print("Unknow error:"+str(e)) 
...
def dailynews_d(Date):
    ...
    for i in range(news_link_daily_len):
        f= open('./collection/'+str(Date)+'-'+str(i)+'result.txt','w')
        try:
            ...
            catch_text_from_page(Text_area,f,str(i)+'/'+str(news_link_daily_len))
            f.close()
        except Exception as e:
            print("Unknow error:"+str(e))
